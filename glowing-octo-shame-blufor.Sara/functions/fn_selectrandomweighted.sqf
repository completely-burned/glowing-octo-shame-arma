...
for "_i" from 0 to ((count _weights) - 1) do
{
	...
	if (_weight > 1) then {
		//debugLog "Log: [selectRandomWeighted] Weights should be less than or equal to 1; weight set to 1!";
		_weight = 1
	};
...
private ["_index", "_valueReturned"];
_index = _weighted call BIS_fnc_selectRandom;

if (typeName _index == typeName 0) then {
